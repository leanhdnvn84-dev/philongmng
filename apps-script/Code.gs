...
function buildProposalDocV8414_(data){
  ...
  const logo=proposalLogoBlobV8414_(),brand=right.getChild(0).asParagraph();brand.setAlignment(DocumentApp.HorizontalAlignment.LEFT).setSpacingAfter(3);if(logo){const image=brand.appendInlineImage(logo),w=image.getWidth()||150,h=image.getHeight()||47;image.setWidth(160);image.setHeight(Math.round(h*160/w));}else{brand.appendText('PHI LONG TECHNOLOGY');styleTextV8414_(brand,15,true,'#D71920');}
  ...
  const contactTbl=right.appendTable(contact.map(function(r){return [r[0],':',r[1]];}));contactTbl.setBorderWidth(0);contactTbl.setColumnWidth(0,50);contactTbl.setColumnWidth(1,8);contactTbl.setColumnWidth(2,132);
