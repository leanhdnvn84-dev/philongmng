...
const V22 = Object.freeze({
  ...
  SHEETS: {
    ...
    chatMessages: 'CHATNOIBO',
    chatDevices: 'PHIEN_CHAT_THIET_BI',
...
// V97 — Chat nội bộ giữa nhân viên. Hệ thống chạy "không đăng nhập thật"
// (mọi người dùng chung 1 link), nên người gửi/nhận được CHỌN từ danh sách
// nhân viên (giống cách chọn "Người thực hiện" ở mọi nơi khác trong app),
// không dựa vào phiên đăng nhập.
// V99.2 — Toàn bộ dữ liệu chat nằm NGAY TRONG SHEET CHÍNH, sheet tên
// "CHATNOIBO" (đúng tên bạn đã tạo sẵn), dùng CHUNG bộ hàm CRUD của cả hệ
// thống (getSheet_/readObjects_/upsertObject_/findById_/updateById_/
// nextId_) — không còn tạo file Sheet riêng, không còn bộ hàm chat*_ riêng.

function ensureChatSchemaV97_() {
  const sheetName = V22.SHEETS.chatMessages;
  const headers = ['ID','NGAY_GIO','ID_NGUOI_GUI','ID_NGUOI_NHAN','NOI_DUNG','DA_DOC'];
  let sh = ss_().getSheetByName(sheetName);
  if (!sh) {
    sh = ss_().insertSheet(sheetName);
    sh.getRange(1, 1, 1, headers.length).setValues([headers]);
  ...
  const existing = getHeaders_(sheetName);
  const missing = headers.filter(function(h){ return existing.indexOf(h) < 0; });
  if (missing.length) {
...
// Gửi 1 tin nhắn. input: {ID_NGUOI_GUI, ID_NGUOI_NHAN, NOI_DUNG}
function sendChatMessage_(user, input) {
  ensureChatSchemaV97_();
  const senderId = String(input.ID_NGUOI_GUI || '').trim();
  const receiverId = String(input.ID_NGUOI_NHAN || '').trim();
  const content = String(input.NOI_DUNG || '').trim();
  if (!senderId) throw new Error('CAN_CHON_NGUOI_GUI');
  if (!receiverId) throw new Error('CAN_CHON_NGUOI_NHAN');
  if (senderId === receiverId) throw new Error('KHONG_THE_TU_NHAN_TIN_CHO_CHINH_MINH');
  if (!content) throw new Error('NOI_DUNG_TRONG');
  if (!findById_(V22.SHEETS.employees, 'ID', senderId)) throw new Error('NGUOI_GUI_KHONG_TON_TAI');
  if (!findById_(V22.SHEETS.employees, 'ID', receiverId)) throw new Error('NGUOI_NHAN_KHONG_TON_TAI');
  const record = {
    ID: nextId_(V22.SHEETS.chatMessages, 'TN'),
    NGAY_GIO: nowStamp_(),
  ...
  upsertObject_(V22.SHEETS.chatMessages, 'ID', record);
  return record;
}

// Lấy toàn bộ hội thoại giữa 2 người, cũ → mới. sinceId (tuỳ chọn): chỉ lấy
// tin nhắn có ID mới hơn, dùng cho polling để không tải lại toàn bộ mỗi lần.
function getChatConversation_(meId, otherId, sinceId) {
  ensureChatSchemaV97_();
  const a = String(meId || '').trim(), b = String(otherId || '').trim();
  if (!a || !b) return [];
  const rows = readObjects_(V22.SHEETS.chatMessages).filter(function(r) {
    const s = String(r.ID_NGUOI_GUI || ''), rcv = String(r.ID_NGUOI_NHAN || '');
    return (s === a && rcv === b) || (s === b && rcv === a);
  }).sort(function(x, y) {
    return String(x.NGAY_GIO || '').localeCompare(String(y.NGAY_GIO || '')) || String(x.ID || '').localeCompare(String(y.ID || ''));
  });
  if (!sinceId) return rows;
  const idx = rows.findIndex(function(r){ return String(r.ID) === String(sinceId); });
  return idx >= 0 ? rows.slice(idx + 1) : rows;
}

// Danh sách đồng nghiệp kèm tin nhắn gần nhất + số tin chưa đọc, để hiển thị
// cột danh sách hội thoại bên trái, sắp xếp theo tin mới nhất trước.
// Bản NHẸ chỉ đếm số tin chưa đọc (1-1 + nhóm) — KHÔNG đọc bảng nhân viên,
// dùng cho polling nền khi popup đang ĐÓNG, để không lặp lại việc đọc toàn
// bộ danh bạ mỗi vài giây một cách lãng phí.
function getChatUnreadCounts_(meId) {
  ensureChatSchemaV97_();
  const me = String(meId || '').trim();
  if (!me) return { direct: 0 };
  const rows = readObjects_(V22.SHEETS.chatMessages);
  let direct = 0;
  rows.forEach(function(r) {
    if (String(r.ID_NGUOI_NHAN) === me && norm_(r.DA_DOC) !== 'TRUE') direct++;
  });
  return { direct: direct };
}

function getChatConversationList_(meId) {
  ensureChatSchemaV97_();
  const me = String(meId || '').trim();
  const employees = readObjects_(V22.SHEETS.employees);
  if (!me) return { employees: employees, threads: [] };
  const messages = readObjects_(V22.SHEETS.chatMessages).filter(function(r) {
    return String(r.ID_NGUOI_NHAN || '') !== CHAT_GROUP_TAG_ && (String(r.ID_NGUOI_GUI) === me || String(r.ID_NGUOI_NHAN) === me);
  });
  const byOther = {};
  messages.forEach(function(r) {
    const other = String(r.ID_NGUOI_GUI) === me ? String(r.ID_NGUOI_NHAN) : String(r.ID_NGUOI_GUI);
    if (!byOther[other] || String(r.NGAY_GIO) > String(byOther[other].NGAY_GIO)) byOther[other] = r;
  });
  const unreadCount = {};
  messages.forEach(function(r) {
    if (String(r.ID_NGUOI_NHAN) === me && norm_(r.DA_DOC) !== 'TRUE') {
      const other = String(r.ID_NGUOI_GUI);
      unreadCount[other] = (unreadCount[other] || 0) + 1;
    }
  });
  const threads = Object.keys(byOther).map(function(otherId) {
    return { ID_NHAN_VIEN: otherId, lastMessage: byOther[otherId], unread: unreadCount[otherId] || 0 };
  }).sort(function(a, b) {
    return String(b.lastMessage.NGAY_GIO || '').localeCompare(String(a.lastMessage.NGAY_GIO || ''));
  });
  return { employees: employees, threads: threads };
}

// Đánh dấu đã đọc toàn bộ tin nhắn otherId đã gửi cho meId.
function markChatRead_(meId, otherId) {
  ensureChatSchemaV97_();
  const me = String(meId || '').trim(), other = String(otherId || '').trim();
  if (!me || !other) return { ok: true, updated: 0 };
  const sh = getSheet_(V22.SHEETS.chatMessages), headers = getHeaders_(V22.SHEETS.chatMessages);
  const idIdx = headers.indexOf('ID_NGUOI_GUI'), rIdx = headers.indexOf('ID_NGUOI_NHAN'), dIdx = headers.indexOf('DA_DOC');
  const lastRow = sh.getLastRow();
  ...
  values.forEach(function(row) {
    if (String(row[idIdx]) === other && String(row[rIdx]) === me && norm_(row[dIdx]) !== 'TRUE') {
      row[dIdx] = 'TRUE';
      updated++;
    }
  });
  if (updated) range.setValues(values);
  invalidateSheetV20_(V22.SHEETS.chatMessages, true);
  return { ok: true, updated: updated };
}

// V100 — Chat NHÓM: 1 luồng tin nhắn DUY NHẤT, mọi nhân viên cùng thấy
// chung (khác với chat 1-1 ở trên). Dùng chung sheet CHATNOIBO, đánh dấu
// bằng giá trị đặc biệt ID_NGUOI_NHAN='__GROUP__' để phân biệt với tin
// nhắn riêng tư 1-1 (không lẫn vào nhau khi đọc hội thoại cá nhân).
var CHAT_GROUP_TAG_ = '__GROUP__';

function sendGroupChatMessage_(user, input) {
  ensureChatSchemaV97_();
  const senderId = String(input.ID_NGUOI_GUI || '').trim();
  const content = String(input.NOI_DUNG || '').trim();
  if (!senderId) throw new Error('CAN_CHON_NGUOI_GUI');
...
// Lấy tin nhắn nhóm, cũ → mới. sinceId (tuỳ chọn): chỉ lấy tin mới hơn,
// dùng cho polling.
function getGroupChatMessages_(sinceId) {
  ensureChatSchemaV97_();
  const rows = readObjects_(V22.SHEETS.chatMessages).filter(function(r) {
    return String(r.ID_NGUOI_NHAN || '') === CHAT_GROUP_TAG_;
  }).sort(function(x, y) {
    return String(x.NGAY_GIO || '').localeCompare(String(y.NGAY_GIO || '')) || String(x.ID || '').localeCompare(String(y.ID || ''));
  });
  if (!sinceId) return rows;
...
// V99 — Ghi nhớ "Bạn đang là ai" theo từng thiết bị/trình duyệt, để mở lại
// trang không phải chọn lại. deviceId là 1 chuỗi ngẫu nhiên vô danh do trình
// duyệt tự sinh 1 lần (lưu trong localStorage), KHÔNG chứa thông tin cá nhân
// — chỉ dùng để tra đúng dòng. Cũng là 1 sheet trong file chính.
function ensureChatDeviceSchemaV99_() {
  const sheetName = V22.SHEETS.chatDevices;
  const headers = ['ID_THIET_BI','ID_NHAN_VIEN','CAP_NHAT_LUC'];
  let sh = ss_().getSheetByName(sheetName);
  if (!sh) {
    sh = ss_().insertSheet(sheetName);
    sh.getRange(1, 1, 1, headers.length).setValues([headers]);
...
function saveChatDeviceIdentity_(deviceId, employeeId) {
  ensureChatDeviceSchemaV99_();
  const device = String(deviceId || '').trim();
  const emp = String(employeeId || '').trim();
  if (!device) throw new Error('THIEU_MA_THIET_BI');
  if (!emp) throw new Error('THIEU_MA_NHAN_VIEN');
  upsertObject_(V22.SHEETS.chatDevices, 'ID_THIET_BI', {
    ID_THIET_BI: device,
    ID_NHAN_VIEN: emp,
    CAP_NHAT_LUC: nowStamp_()
  });
  return { ok: true };
}

function getChatDeviceIdentity_(deviceId) {
  ensureChatDeviceSchemaV99_();
  const device = String(deviceId || '').trim();
  if (!device) return { ID_NHAN_VIEN: '' };
  const row = findById_(V22.SHEETS.chatDevices, 'ID_THIET_BI', device);
  return { ID_NHAN_VIEN: row ? String(row.ID_NHAN_VIEN || '') : '' };
}
...
function legacyV22DispatchNoAuth_(fn, args, user) {
  switch (fn) {
    ...
    case 'sendChatMessage': return sendChatMessage_(user, args[0]||{});
    case 'sendGroupChatMessage': return sendGroupChatMessage_(user, args[0]||{});
    case 'getGroupChatMessages': return getGroupChatMessages_(args[0]||'');
    case 'getChatConversation': return getChatConversation_(args[0]||'', args[1]||'', args[2]||'');
    case 'getChatConversationList': return getChatConversationList_(args[0]||'');
    case 'getChatUnreadCounts': return getChatUnreadCounts_(args[0]||'');
    case 'markChatRead': return markChatRead_(args[0]||'', args[1]||'');
    case 'saveChatDeviceIdentity': return saveChatDeviceIdentity_(args[0]||'', args[1]||'');
    case 'getChatDeviceIdentity': return getChatDeviceIdentity_(args[0]||'');
