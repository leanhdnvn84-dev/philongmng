...
// Đường kẻ đen dưới logo: ảnh PNG 1x1 màu đen, kéo giãn thành vạch ngang.
const PRINT_RULE_PNG_B64_V90='iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1PeAAAADElEQVR4nGNgYGAAAAAEAAH2FzhVAAAAAElFTkSuQmCC';
...
function buildProposalDocV8414_(data){
  ...
  const logo=proposalLogoBlobV8414_(),brand=right.getChild(0).asParagraph();right.setPaddingRight(0);brand.setAlignment(DocumentApp.HorizontalAlignment.RIGHT).setSpacingAfter(0);if(logo){const image=brand.appendInlineImage(logo),w=image.getWidth()||640,h=image.getHeight()||200;image.setWidth(183);image.setHeight(Math.round(h*183/w));}else{brand.appendText('PHI LONG TECHNOLOGY');styleTextV8414_(brand,15,true,'#D71920');}const rule=right.appendParagraph('');rule.setAlignment(DocumentApp.HorizontalAlignment.RIGHT).setSpacingBefore(1).setSpacingAfter(3);const ruleImg=rule.appendInlineImage(Utilities.newBlob(Utilities.base64Decode(PRINT_RULE_PNG_B64_V90),'image/png','rule.png'));ruleImg.setWidth(205);ruleImg.setHeight(1);
