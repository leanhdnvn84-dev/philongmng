...
function buildProposalDocV8414_(data){
  ...
  left.setPaddingBottom(0);right.setPaddingBottom(0);const hrPara=body.appendHorizontalRule().getParent().asParagraph();hrPara.setSpacingBefore(2).setSpacingAfter(0).setLineSpacing(1);hrPara.editAsText().setFontSize(1);
  const recipient=body.appendTable([['Kính gửi:','- Giám đốc Công ty TNHH Công Nghệ Tin Học Phi Long\n- Bộ phận quản lý']]);recipient.setBorderWidth(0);recipient.setColumnWidth(0,70);recipient.setColumnWidth(1,450);styleCellV8414_(recipient.getCell(0,0),10,true,'#FFFFFF',DocumentApp.HorizontalAlignment.LEFT);styleCellV8414_(recipient.getCell(0,1),10,true,'#FFFFFF',DocumentApp.HorizontalAlignment.LEFT);recipient.getCell(0,0).setVerticalAlignment(DocumentApp.VerticalAlignment.TOP);recipient.getCell(0,0).setPaddingTop(3);recipient.getCell(0,1).setPaddingTop(3);recipient.getCell(0,1).setVerticalAlignment(DocumentApp.VerticalAlignment.TOP);
